...
**HospitalCore** — HospitalCore's bounded Morbidity Reporting subcontext consumes this context's immutable diagnosis facts through the named scalar capability and may cross-reference them with HospitalCore-owned historical registration, patient, and reference facts while processing. HospitalCore keeps its own append-only source-fact record and derived report view; its persisted and report-facing result is opaque, exposing only aggregates or opaque reporting references. This context remains the sole owner of Diagnosis Assertions, their lineage, and their source timestamps. The reporting projection does not become a diagnosis store, clinical-record layer, or direct reader of these tables.
